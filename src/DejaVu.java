...
public class DejaVu {
    ...
    public int mostDejaVus(int n, int seed, int R) {
        ...
        List<List<DejaVu.Query>> queries = new ArrayList<>();
        for (int i = 0; i < n; i++) {
        ...
        for (Map.Entry<Integer, List<Integer>> entry : mp.entrySet()) {
            ...
            for (int i = 0; i < idxs.size() - 1; i++) {
                ...
                queries.get(prevEnd).add(new DejaVu.Query(0, last, until));
                queries.get(first + 1).add(new DejaVu.Query(1, last, until));
                prevEnd = first + 1;
            }
        }

        DejaVu.SegmentTree tree = new DejaVu.SegmentTree(n);
        int ans = Integer.MIN_VALUE;

        for (int i = 0; i < n; i++) {
            for (DejaVu.Query query : queries.get(i)) {
                if (query.type == 0) {
    ...
    public static class SegmentTree {
        ...
        public DejaVu.SegmentTree.Data[] t;

        public SegmentTree(int n) {
            this.t = new DejaVu.SegmentTree.Data[n * 4 + 1];
            for (int i = 0; i < n * 4 + 1; i++) {
                this.t[i] = new DejaVu.SegmentTree.Data();
            }
        }
        ...
        public static class Data {
            ...
            public DejaVu.SegmentTree.Data combine(DejaVu.SegmentTree.Data a, DejaVu.SegmentTree.Data b) {
